...
>
  <div className="flex items-center flex-col sm:flex-row md:justify-end gap-2 sm:gap-10 pr-10 py-2 bg-primary">
    ...
    <div className="flex items-center gap-3">
      ...
      <p className="font-bold text-black">+91 96550 99911</p>
    </div>

    <div className="flex items-center gap-3">
      <Mail className="text-black" />
      <p className="font-bold text-black">
        info@Ravananpackersandmovers.com
      </p>
    </div>
...
>
  ...
  {/* target */}
  <div className={`${showTopBar ? controlTopBar ? "bg-black" : "" : "bg-black"} duration-500 flex justify-between items-center px-10 py-4`}>
 ...
 <ul className="hidden lg:flex gap-10 font-semibold">
  {menuItems.map((item, i) => (
    ...
    `transition flex items-center gap-1 ${
      ...
      {/* ===== DROPDOWN ===== */}
      {item.child && (
        <ul className="absolute left-0 top-full mt-4 w-56 bg-black border-white border shadow-xl rounded-xl 
          ...
          {item.child.map((sub, idx) => (
            <li key={idx}>
              <NavLink
                to={sub.path}
                className="block px-5 py-3 text-white hover:bg-black hover:text-yellow-300 transition rounded-xl"
              >
                {sub.name}
              </NavLink>
            </li>
...
>
  ...
  `}
>
  <div className="bg-black shadow-lg rounded-b-2xl border-t border-amber-200 mt-1 z-50 isolate  ">
